...
local enchantmentMaxChargeCache = {}
...
local function getMaxEnchantmentCharge(enchantment)
    ...
    if enchantmentMaxChargeCache[enchantment.id] then
    ...
    if enchantment.type == core.magic.ENCHANTMENT_TYPE.ConstantEffect then
        enchantmentMaxChargeCache[enchantment.id] = 0
        return enchantmentMaxChargeCache[enchantment.id]
    end
    ...
    if enchantment.isAutocalc then
        ...
        enchantmentMaxChargeCache[enchantment.id] = baseCost * chargeMultiplier
        return enchantmentMaxChargeCache[enchantment.id]
    end

    -- Otherwise, use the stored charge value from the enchantment record
    enchantmentMaxChargeCache[enchantment.id] = enchantment.charge
    return enchantmentMaxChargeCache[enchantment.id]
end
