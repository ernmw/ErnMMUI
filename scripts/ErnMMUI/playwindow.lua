...
local sprite         = require("scripts.ErnMMUI.render.sprite")
local keytrack       = require("scripts.ErnMMUI.keytrack")
local trajectory     = require("scripts.ErnMMUI.render.trajectory")
...
local resilientShine = sprite.NewAnimatedImage("textures\\ErnMMUI\\circle-sweep.dds",
...
local function makeExplodeAnim(props)
    return sprite.NewAnimatedImage("textures\\ErnMMUI\\blast.dds",
...
local function makeHitAnim(left, props)
    return sprite.NewAnimatedImage(
        left and "textures\\ErnMMUI\\hit_left.dds" or "textures\\ErnMMUI\\hit_right.dds",
        util.vector2(2 * 256, 2 * 256),
        4, 20, 1, nil, props)
end
...
function PlayWindow:_onPinHit(ballId, pinId)
    ...
    ambient.playSoundFile("Sound\\ErnMMUI\\boink.ogg")
